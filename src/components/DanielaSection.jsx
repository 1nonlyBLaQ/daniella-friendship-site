...
export default function DanielaSection({ onNext }) {
  const videos = siteConfig.videosDaniela;
  ...
  return (
    <div className="cute-card p-6 sm:p-8 rounded-3xl page-transition max-w-md mx-auto">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h2 className="text-xl font-serif font-bold text-stone-900">And now… you</h2>
          <p className="text-stone-400 text-xs">Slide {currentIndex + 1} of {videos.length}</p>
        </div>
        <span className="px-3 py-1 bg-rose-100 text-rose-600 text-xs font-bold rounded-full">Main Character ✨</span>
      </div>

      {/* PowerPoint Wipe Animation Container */}
      <div key={currentIndex} className="animate-wipe-next">
        <div className="w-full aspect-[9/16] max-h-[380px] mx-auto bg-stone-950 rounded-2xl overflow-hidden shadow-lg relative flex items-center justify-center mb-4">
          <video 
            controls 
            preload="metadata"
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <span>{currentIndex === videos.length - 1 ? "Final Note →" : "Next Slide"}</span>
